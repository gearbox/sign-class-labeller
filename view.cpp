...
QLabel *make_label(QLabel **label, const QString &name, int width = -1,
  ...
  *label = new QLabel(name);
  ...
  if (width != -1) (*label)->setFixedWidth(width);
...
QSpacerItem *make_spacer(int width) { return new QSpacerItem(width, 1); }
...
#define BIND(shortcut_member, seq_string, slot_fun)                \
  shortcut_member = new QShortcut(QKeySequence(seq_string), this); \
...
void View::bind_signals() {
  ...
  connect(unknown_button_, SIGNAL(clicked()), this, SIGNAL(next_unknown()));
...
QVector<tuple<QString, bool>> View::get_marks() {
  ...
  auto items =
      sign_imgs_->findItems(QString("*"), Qt::MatchWrap | Qt::MatchWildcard);
  for (auto item : items)
...
void View::set_back_enabled(bool value) { back_button_->setEnabled(value); }
...
int View::input_number() {
  bool ok;
  int number = QInputDialog::getInt(this, "Input number", "Number on sign:", 1,
                                    1, 1000, 1, &ok);
  return (ok ? number : -1);
}
